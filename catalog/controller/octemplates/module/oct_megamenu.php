	            ...
	            foreach ($results as $result) {
	                ...
	                if ($result['item_type'] == 3) {
	                    ...
	                    if ($children_data) {
		                    ...
		                    array_multisort($i_sort_order, SORT_ASC, SORT_NATURAL, $childrens, SORT_ASC, SORT_NATURAL);
        
                            $first_letters_arr = [];
                            foreach($i_sort_order as $word){
                                $first_letter = mb_strtoupper(mb_substr(trim($word), 0, 1));
                                $first_letters_arr[] = $first_letter;
                            }
                            $first_letters_arr = array_unique($first_letters_arr);
                            sort($first_letters_arr);
                            
                            $brands_arr = [];
                            foreach ($first_letters_arr as $first_letter){
                                foreach ($childrens as $child){
                                    if( $first_letter == mb_strtoupper(mb_substr(trim($child['name']), 0, 1)) ){
                                        $brands_arr[$first_letter][] = $child;
                                    }
                                }
                            }
                            
                            $childrens = $brands_arr;
